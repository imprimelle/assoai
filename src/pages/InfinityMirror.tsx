...
import Mirror3D, { type LedType } from "@/components/infinity-mirror/Mirror3D";
...
const calcLedCount = (perimeter: number, density: number): number => Math.round(perimeter * density);

const calcReflections = (R_f: number, R_m: number): number => {
  let n = 0, intensity = 100;
  while (intensity >= 1) {
...
const LED_COLORS: { name: string; hex: string; css: string }[] = [
...
const LED_TYPES: { key: LedType; label: string; desc: string }[] = [
  { key: "ruban", label: "Ruban", desc: "Bande LED continue" },
  { key: "module", label: "Module", desc: "LEDs individuelles" },
  { key: "neon", label: "Néon", desc: "Tubes fluorescents" },
];
        ...
        <div className="absolute bottom-4 right-4 z-10">
          {showBrightness && (
            <div className="absolute bottom-12 right-0 bg-black/80 backdrop-blur-md rounded-xl p-3 border border-white/10 mb-2 w-48">
              ...
              <Slider value={[brightness]} min={0.3} max={2.5} step={0.1} onValueChange={([v]) => setBrightness(v)} className="w-full" />
              <button onClick={() => setBrightness(1.0)} className="mt-2 w-full text-[10px] text-gray-500 hover:text-white transition-colors">Réinitialiser</button>
            </div>
          )}
      ...
      {createPortal(
        <div ref={footerRef} className="fixed bottom-0 left-0 right-0 bg-[#0f0f1a]/95 backdrop-blur-md border-t border-white/10 shadow-lg z-50">
          ...
          <div className="flex items-center gap-2 px-3 py-2">
            ...
            <button onClick={resetAll} className="p-1.5 rounded-lg text-gray-500 hover:text-white hover:bg-white/5 transition-colors" title="Réinitialiser">
            ...
            <div className="px-3 pb-3 space-y-3 max-h-[50vh] overflow-y-auto">
              ...
              {activeTab === "led" && (
                <div className="space-y-3">
                      ...
                      {LED_DENSITIES.map((dens) => (
                        <button key={dens} onClick={() => setD_led(dens)}
                          className={`flex-1 py-2 rounded-lg text-sm font-medium transition-all ${
                            D_led === dens ? "bg-brand-orange text-white shadow-lg shadow-brand-orange/20" : "bg-white/5 text-gray-400 hover:bg-white/10 hover:text-white"
                          }`}>
                      ...
                      {LED_TYPES.map((t) => (
                        <button key={t.key} onClick={() => setLedType(t.key)}
                          className={`flex-1 py-2 rounded-lg text-xs font-medium transition-all ${
                            ledType === t.key ? "bg-white/10 text-white border border-white/20" : "bg-white/5 text-gray-400 hover:bg-white/10 hover:text-white"
                          }`}>
                          <div>{t.label}</div>
                          <div className="text-[9px] opacity-50">{t.desc}</div>
                        </button>
                      ))}
                    </div>
                  </div>
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {LED_COLORS.map((c) => (
                        <button key={c.hex} onClick={() => setLedColor(c.hex)}
                          className={`w-7 h-7 rounded-full border-2 transition-all ${c.css} ${
                            ledColor === c.hex ? "border-white scale-110 shadow-lg" : "border-transparent opacity-70 hover:opacity-100"
                          }`}
                          title={c.name} />
                      ))}
                    </div>
                  ...
                  {/* Puissance */}
                  <CompactSlider label="Puissance" value={ledPower} min={4.8} max={28.8} step={0.1} unit="W/m"
                    onChange={setLedPower}
                    hint="4.8W (30LED/m) → 28.8W (120LED/m)" />
                </div>
              )}
              ...
              {activeTab === "optique" && (
                <div className="space-y-3">
                  <CompactSlider label="Film sans tain (R_f)" value={R_f} min={50} max={95} unit="%" onChange={setR_f}
                    hint="Réflectivité du verre supérieur" />
                  ...
                  <div className="bg-white/5 rounded-lg p-3 space-y-2">
                    ...
                    <div className="grid grid-cols-2 gap-2 text-xs">
                      <div>
                        <span className="text-gray-500">Intensité initiale</span>
                        <div className="font-mono text-white">100%</div>
                      </div>
                      <div>
                        <span className="text-gray-500">Intensité finale</span>
                        <div className="font-mono text-white">
                          {reflections > 0
                            ? `${(100 * Math.pow(R_f / 100, reflections) * Math.pow(R_m / 100, reflections)).toFixed(2)}%`
                            : "—"}
                        </div>
                      </div>
                      <div>
                        <span className="text-gray-500">Réflexions visibles</span>
                        <div className="font-mono text-cyan-400 font-bold">{reflections}</div>
                      </div>
                      <div>
                        <span className="text-gray-500">Perte par réflexion</span>
                        <div className="font-mono text-white">{((1 - (R_f / 100) * (R_m / 100)) * 100).toFixed(1)}%</div>
                      </div>
                    </div>
                  </div>
                </div>
...
const PhysicalNote: React.FC<{ d: number; visualDepth: number; reflections: number }> = ({ d, visualDepth, reflections }) => (
  <div className="bg-white/5 rounded-lg p-2.5 text-xs space-y-1 border border-white/5">
    <div className="flex justify-between">
      <span className="text-gray-500">Profondeur physique</span>
      <span className="font-mono text-white font-semibold">{d} cm</span>
    </div>
    <div className="flex justify-between">
      <span className="text-gray-500">Gouffre visuel perçu</span>
      <span className="font-mono text-purple-400 font-semibold">{visualDepth} cm</span>
    </div>
    <div className="text-[10px] text-gray-600 mt-1">
      La structure fait <strong className="text-white">{d} cm</strong> d'épaisseur. L'illusion d'optique donne l'impression de <strong className="text-purple-400">{visualDepth} cm</strong> de profondeur grâce aux {reflections} réflexions entre le miroir et le film sans tain.
    </div>
  </div>
);
