...
const PhotoRequiredDialog: React.FC<PhotoRequiredDialogProps> = ({
  ...
  return (
    <>
      {/* CameraCaptureUI (getUserMedia natif prioritaire) */}
      <CameraCaptureUI
        open={showCameraUI}
          ...
          >
            ...
            {!uploaded ? (
              <div className="flex items-center justify-center gap-6">
                {/* Appareil photo → CameraCaptureUI avec getUserMedia natif */}
                <button
                  type="button"
            ...
            {/* Input galerie (fallback) */}
            <input ref={galleryRef} type="file" className="hidden" accept="image/*" onChange={handleGalleryChange} />
          </div>
        </div>,
        document.body,
      )}
