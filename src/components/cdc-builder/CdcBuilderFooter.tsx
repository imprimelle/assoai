...
1. Une courte analyse expliquant ce que tu modifies, pourquoi, et les impacts.
   Structure avec du markdown : **gras** pour les points importants, - listes pour énumérer, ### pour les sections.
...
⚠️ Tu es en mode DISCUSSION (pas en mode modification). Réponds de façon conversationnelle et structurée.
   Format conseillé :
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (mode === "modifier") {
        ...
        setMessages((prev) => [
          ...prev,
          { role: "ai", agent: "brico", text: parsed.message || (parsed.actions?.length ? "✅ Modifications appliquées." : responseText) },
        ]);
        if (parsed.actions?.length) {
